    ...
    <div class="form-dropdown-wrapper">
        <div class="form-select-wrapper" id="select" on:click={toggleDropdown}>
            ...
            {:else}
                <div></div>
            {/if}
            <i class="fas fa-chevron-down form-dropdown-chevron"></i>
        </div>
        {#if isOpen}
            <div class="form-options-wrapper">
                {#if dropdownValues.length < 1}
                    <div class="form-no-items-found">
                        No items found
                    </div>
                {:else}
                    {#each dropdownValues as ddValue}
                        <div class="form-option-child" on:click={() => {selectDropdownValue(ddValue)}}>
                            <p>
                                {ddValue} 
